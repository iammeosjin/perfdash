 ...
 * @version     2.1.0
 * @file        dataTables-3.7.1.responsive.js
...
(function (factory) {
	...
	$.extend(Responsive.prototype, {
		...
		_columnsVisiblity: function (breakpoint) {
			...
			for (i = 0, ien = order.length; i < ien; i++) {
				...
				if (
					display[colIdx] === '-' && !columns[colIdx].control &&
		...
		_resize: function () {
			...
			for (i = 0, ien = columns.length; i < ien; i++) {
				if (
					columnsVis[i] === false && !columns[i].never &&
					!columns[i].control
				) {
					collapsedClass = true;
					break;
		...
		_resizeAuto: function () {
			...
			var clonedBody = $(dt.table().body()).clone(false, false).empty()
		...
		_tabIndexes: function () {
			...
			cells.filter('[data-dtr-keyboard]').removeData(
				'[data-dtr-keyboard]',
			);
	...
	Responsive.display = {
		...
		childRowImmediate: function (row, update, render) {
			if (
				(!update && row.child.isShown()) || !row.responsive.hasHidden()
			) {
