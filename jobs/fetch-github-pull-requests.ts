...
import calculateCycleMetricsPoints from '../libs/calculate-cycle-metrics-points.ts';
...
export default function fetchGithubPullRequests(teams: Team[]) {
	...
	return Bluebird.mapSeries(teams, async (team) => {
		...
		const response = await consumePagination({
			weeklySummary: {},
		}, {
			team,
...
async function consumePagination(
	...
	if (refetch) {
		return consumePagination(response, {
			...options,
			after,
		});
	}
